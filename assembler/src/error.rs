...
pub fn invalid_token(unit_path: &Path, token: &Token, line_number: usize, line: &str, hint: &str) -> ! {
    println!(
"Error in assembly unit \"{}\"
...
{}",
unit_path.display(), token, line_number, line, hint
    );
    std::process::exit(1);
}


pub fn invalid_token_argument(unit_path: &Path, instruction: &str, arg: &Token, line_number: usize, line: &str) -> ! {
    println!(
"Error in assembly unit \"{}\"
Invalid argument \"{}\" for instruction `{}` at line {}:
{}",
unit_path.display(), arg, instruction, line_number, line
    );
    std::process::exit(1);
}
