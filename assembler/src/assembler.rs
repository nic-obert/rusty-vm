...
pub fn load_unit_asm<'a>(unit_path: UnitPath<'a>, symbol_table: &SymbolTable<'a>, module_manager: &'a ModuleManager<'a>, bytecode: &mut ByteCode, debug: bool) -> Option<Box<[AsmNode<'a>]>> {
    ...
    let asm = parser::parse(token_lines, symbol_table, module_manager, bytecode, debug);

    Some(asm)
}


pub fn assemble_included_unit<'a>(unit_path: UnitPath<'a>, module_manager: &'a ModuleManager<'a>, bytecode: &mut ByteCode, debug: bool) -> &'a ExportedSymbols<'a> {

    let symbol_table = SymbolTable::new();

    let asm = if let Some(asm) = load_unit_asm(unit_path, &symbol_table, &module_manager, bytecode, debug) {
        asm
    } else {
        return module_manager.get_unit_exports(unit_path);
    };

    generator::generate_bytecode(asm, &symbol_table, module_manager, bytecode, debug);
...
pub fn assemble_all(caller_directory: &Path, unit_path: &Path, include_paths: Vec<PathBuf>, debug: bool) -> ByteCode {
    ...
    let asm = load_unit_asm(unit_path, &symbol_table, &module_manager, &mut bytecode, debug)
        .expect("Main ASM unit should not be already loaded");

    generate_bytecode(asm, &symbol_table, &module_manager, &mut bytecode, debug);
